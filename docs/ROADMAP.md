...
- [x] **Capacité sans heure : la fenêtre de phase** (fait) : certaines capacités n'ont pas de timing du tout, juste une fenêtre où elles deviennent possibles — le souffle d'Onyxia pendant le vol, dont la probabilité tient à la durée de la phase 2, donc au DPS. `variable` + `pendingWindow = "phase"` sur un timer `PHASE` (`time = 0` = possible dès l'entrée) : voyant « c'est possible » affiché toute la phase, réarmé derrière chaque occurrence, fermé au changement de phase, et une annonce uniquement sur le cast observé. La régénération `wcl-ingest` laisse ces timers intacts — un log mesurera toujours une médiane et une cadence pour ce sort, et les écrire rendrait au fichier l'échéance fausse qu'on vient d'en retirer.
